...
public class SimplifiedIndexPqTest extends AppTestContext {
    @Autowired
    private PriorityComparator<Integer> minIntPriorityComparator;

    @Test
    public void testIndexedPq() {
        SimplifiedIndexedPq<Integer> indexedMinPq = new SimplifiedIndexedPq(minIntPriorityComparator, 16);
    ...
    public void testContains() {
        SimplifiedIndexedPq indexedMinPq = new SimplifiedIndexedPq(minIntPriorityComparator, 16);
        repopulate(indexedMinPq);
    ...
    private void assertDeleteMin(SimplifiedIndexedPq indexedMinPq, int expectedMinId) {
        int minId = indexedMinPq.delMin();
        Assert.assertEquals("delMin", expectedMinId, minId);
    }

    private void repopulate(SimplifiedIndexedPq indexedMinPq) {
